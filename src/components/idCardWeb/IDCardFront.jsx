export default function IDCardFront({ data }) {
  return (
    <div
      className=" rounded-lg shadow-lg flex overflow-hidden bg-bg"
      style={{ width: 360, height: 220 }}
    >
      {/* LEFT PANEL: PROFILE PHOTO */}
      <div
        className="flex items-center justify-center bg-bg p-2"
      ...
      <div className="flex flex-col justify-between flex-1 p-3 relative">
        ...
        <div className="flex flex-col justify-center h-full ml-2 space-y-0.5 pr-14">
          <p className="text-lg font-bold text-textColor truncate">{data.name}</p>
          ...
          {data.type && (
            <p className="text-xs font-medium text-textColor truncate">
              {data.type}
            </p>
          )}
        </div>
        ...
        {/* DIVIDER */}
        <div style={{ height: 1, backgroundColor: "#9ca3af" }} className="mt-1"></div>

        {/* FOOTER */}
        {data.company && (
          <p className="text-xs text-textColor text-center mt-1 px-1 break-words">
            {data.company}
          </p>
        )}
      </div>
